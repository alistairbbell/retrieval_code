...
%
function L2 = qpack2_retrieval_nedoluha(M,R,QQ)
%%% Init atmlab
...
if M.recalc_noise % calculate noise using a first guess retrieval
    Y.TNOISE     = .1;
end
...
if M.recalc_noise
    Y.TNOISE = std(L2.y-L2.yf);

    %%% Make 2nd inversion
    disp('Doing 2nd inversion...')
    L2 = qpack2( Q, O, Y, @arts_oem_bal );
    if isempty(L2)
        disp('Could not do the 2nd inversion!')
        return
    end
end
...
if strcmp(Q.ABS_SPECIES(1).UNIT,'rel')||strcmp(Q.ABS_SPECIES(1).UNIT,'logrel') 
    ...
    A             = L2.species1_A;
    mr            = L2.species1_mr;
    L2.char.erel  = L2.species1_e./L2.species1_xa;
    L2.char.eorel  = L2.species1_eo./L2.species1_xa;
    L2.char.esrel  = L2.species1_es./L2.species1_xa;
end
...
L2.char.ap     = get_avk_params(A,mr,L2.species1_p,L2.species1_z,n);
L2.char.offset = L2.polyfit0_x;
if L2.R.polyfit_order>0
...
Y.ZA         = msm.za_line;      % Zenith angle of the measurement [deg]
