function ptz=get_ptz_MLSclimat(time_num)

% select data
C=mysql('select * from atmospheres.MLS_Aura_T_climatology','mat');

% reshape data
p=reshape(C(:,4),34,365);
T=reshape(C(:,5),34,365);
z=reshape(C(:,6),34,365);
