...
%
function L2 = qpack2_retrieval_tr(M,R,QQ)

%%% Init atmlab
if ~exist('atmlab.m','file') || isempty(strfind(which('arts.m'),'atmlab')) || sum(isnan(atmlab('ARTS_PATH'))) || ~isempty(strfind(which('arts.m'),'balancing'))
...
if M.recalc_noise||isempty(msm.sigma) % calculate noise using a first guess retrieval
    Y.TNOISE     = .1;
end
