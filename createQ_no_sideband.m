% function Q = createQ_no_sideband(Y,R)
%
...
Q.ABS_LINES            = fullfile(curr_dir,'datafiles','lines_nedoluha1995_finesplitting_only_H2O.xml');
...
switch R.PTZ_profile
    case 'ECMWF'
        if ~isempty(R.PTZ_profile_hour_interval)
    ...
    case 'ECMWF_nodiurnal'
        ptz = get_ptz_ECMWF_temporal_interpolation(datestr((Y.MIN_TIME/2+Y.MAX_TIME/2-1),31),datestr((Y.MIN_TIME/2+Y.MAX_TIME/2+1),31),Y.LATITUDE,Y.LONGITUDE);
    case 'USSTD'
